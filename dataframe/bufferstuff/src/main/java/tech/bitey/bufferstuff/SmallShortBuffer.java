...
/**
 * This class has an API similar to {@link ShortBuffer}. All implementations are
 * backed by a {@link BigByteBuffer}. "Small" refers to the fact that these
 * buffers are indexed by ints rather than longs.
 * <p>
 * Differences from {@code ShortBuffer} include:
 * <ul>
...
public final class SmallShortBuffer extends SmallBuffer {
	...
	public SmallShortBuffer put(int index, short value) {
		buffer.putShort((long) index << SHIFT, value);
		return this;
	}
	...
	public short get(int index) {
		return buffer.getShort((long) index << SHIFT);
	}
	...
	public SmallShortBuffer duplicate() {
		return new SmallShortBuffer(buffer.duplicate());
	}

	@Override
	public SmallShortBuffer slice() {
		return new SmallShortBuffer(buffer.slice());
	}
